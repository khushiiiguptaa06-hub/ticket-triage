...
def main():
    ...
    mock_tickets = df[["title", "description", "category", "urgency"]].to_dict(orient="records")
    
    cls = SklearnClassifier()
    cls.train(mock_tickets)
    print("Trained on mock data")

    test_tickets = Ticket(title="Billing issue", description="Was charged twice this month")
